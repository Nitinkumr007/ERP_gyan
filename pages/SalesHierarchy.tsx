...
import { UserAccessMaster, Distributor } from '../types';
...
const SalesHierarchy: React.FC<SalesHierarchyProps> = () => {
    ...
    // Mappings: userId -> managerId (both as strings to match schema text type, though userId is int)
    ...
    const handleSaveMappings = async () => {
        ...
        setIsSaving(true);
        try {
            // Bulk update not directly supported in a simple way for different values, so we iterate
            // Optimisation: Only update changed values
            ...
            await fetchData();
            setActiveTab('VISUAL'); // Switch to visual tab to show result
    ...
    // Recursive Tree Renderer with Cycle Detection
    ...
    const renderVisualTab = () => {
        ...
        // 1. Has NO manager assigned in map OR
        // 2. Has a manager assigned, BUT that manager is not in the current users list (Ghost Manager)
        // 3. Reports to themselves (Cycle of 1) - though we should filter those out or treat as root to show error is better?
        ...
        const roots = users.filter(u => {
            ...
            // Root if no manager OR manager not found in user list (Ghost) OR reports to self
        ...
        <div className="bg-[var(--bg-panel)] p-6 rounded-2xl border border-[var(--border-color)]">
            <div className="flex justify-between items-end mb-6 sticky top-0 bg-[var(--bg-panel)] z-20 pb-4 border-b border-[var(--border-color)]">
                ...
                <div className="flex gap-2">
                    <button
                        onClick={fetchData}
                        className="p-2.5 bg-[var(--bg-primary)] border border-[var(--border-color)] text-[var(--text-primary)] rounded-xl hover:bg-[var(--bg-secondary)] transition-all"
                        title="Refresh Data"
                    >
                    ...
                    <button
                        ...
                        disabled={isSaving}
                        className="px-6 py-2.5 bg-[var(--color-primary)] hover:opacity-90 text-white rounded-xl font-bold shadow-lg shadow-blue-500/30 transition-all flex items-center gap-2"
                    >
    ...
    const updateDBTerritory = async (dbId: number, asmName: string) => {
        // await supabase.from('Distributor_Master').update({ ASM: asmName }).eq('DB ID', dbId);
        alert(`Mapped Distributor ${dbId} to ASM ${asmName} (Simulation)`);
    };
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 bg-[var(--bg-panel)] z-10 shadow-sm">
                            <tr className="border-b border-[var(--border-color)] text-[var(--text-secondary)] text-xs uppercase tracking-wider">
                                <th className="p-4">
                                    <div className="mb-2">Distributor</div>
                                    <input
                                        type="text"
                        ...
                        <tbody className="divide-y divide-[var(--border-color)]">
                            {filteredDistributors.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="p-8 text-center text-[var(--text-secondary)]">No distributors match your filters.</td>
                                </tr>
                            ) : (
                                filteredDistributors.map(db => (
                                    <tr key={db['DB ID']} className="hover:bg-[var(--bg-secondary)] transition-colors">
                                        <td className="p-4 font-bold text-[var(--text-primary)]">{db['NAME OF DISTRIBUTOR']}</td>
                                        ...
                                        <td className="p-4">
                                            <select
                                                ...
                                                defaultValue={db['ASM'] || ''}
                                                onChange={(e) => updateDBTerritory(db['DB ID'], e.target.value)}
                                            >
                                        ...
                                        <td className="p-4">
                                            <button className="text-[var(--color-primary)] font-bold text-xs hover:underline">View History</button>
                                        </td>
                                    </tr>
                                ))
                            )}
